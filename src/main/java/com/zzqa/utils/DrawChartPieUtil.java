...
public class DrawChartPieUtil {
    ...
    public static File getImageFile(String proName,int normalPart,int warningPart,int alarmPart)
    {
        ...
        //设置片区颜色(未起作用)
//        mPiePlot.setSectionPaint("故障", new Color(192,80,77));
//        mPiePlot.setSectionPaint("亚健康", new Color(155,187,89));
//        mPiePlot.setSectionPaint("健康", new Color(79,129,189));
        //设置背景色
        mPiePlot.setBackgroundPaint(ChartColor.WHITE);
        //数据区边框隐藏
        mPiePlot.setOutlineVisible(false);

        mPiePlot.setLabelGenerator(new StandardPieSectionLabelGenerator("{0}:{1}", NumberFormat.getNumberInstance(), new DecimalFormat("0.00%")));
    ...
    /**
     * 设置饼状图的数据
     * @param normalPort 健康机组数量
     * @param warningPort 亚健康机组数量
     * @param alarmPort 故障机组数量
     * @return 返回一个饼状图数据源
     */
    private static PieDataset GetDataset(int normalPort,int warningPort,int alarmPort)
    {
        ...
        mDataset.setValue(" 故障", new Double(alarmPort));
        mDataset.setValue(" 亚健康", new Double(warningPort));
        mDataset.setValue(" 健康", new Double(normalPort));
        return mDataset;
    }
}
